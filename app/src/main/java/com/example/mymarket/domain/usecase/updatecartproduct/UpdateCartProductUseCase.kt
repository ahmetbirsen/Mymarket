package com.example.mymarket.domain.usecase.updatecartproduct
...
class UpdateCartProductUseCase @Inject constructor(
    ...
    operator fun invoke(product: CartProduct): Flow<Resource<Unit>> = flow {
        try {
            emit(Resource.Loading())
            productRepository.updateProductCount(product)
