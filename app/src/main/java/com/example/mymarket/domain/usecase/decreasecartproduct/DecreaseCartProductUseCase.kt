package com.example.mymarket.domain.usecase.decreasecartproduct
...
class DecreaseCartProductUseCase @Inject constructor(
    private val productRepository: ProductRepository
) {
    operator fun invoke(productId: String): Flow<Resource<Unit>> = flow {
        try {
            emit(Resource.Loading())
            productRepository.decreaseProductCount(cartProductId = productId)
