package com.example.mymarket.domain.usecase.increasecartproduct
...
class IncreaseCartProductUseCase @Inject constructor(
    private val productRepository: ProductRepository
) {
    operator fun invoke(productId: String): Flow<Resource<Unit>> = flow {
        try {
            emit(Resource.Loading())
            productRepository.increaseProductCount(cartProductId = productId)
