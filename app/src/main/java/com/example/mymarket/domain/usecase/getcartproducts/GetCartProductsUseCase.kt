package com.example.mymarket.domain.usecase.getcartproducts
...
class GetCartProductsUseCase @Inject constructor(
    ...
    operator fun invoke() : Flow<Resource<List<CartProduct>>> = flow {
        try {
            emit(Resource.Loading())
            val productsFromCart = productRepository.getCartProducts().first()
